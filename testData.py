...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="testDataPython", description="Run the data testing "
    )
    ...
    parser.add_argument(
        "--online", action="store_true", help="Whether to run for head to head battle"
    )
    ...
    if args.matlabmodel:
        try:
            ...
            print(
                "Matlab engine not found. If using MATLAB, please install matlab engine for python"
            )
        ...
        print(pth)
        print(
            "Make sure your model is called runMatlabModel.m and there is no runMatlabModel.m in the same folder as testData.py"
        )
        eng.addpath(pth)
    ...
    while not stream_name:
        ...
        inp = input(
            "Which stream is your wristband? (r) to reload list of available/ (q) to quit"
        )
        if inp == "r":
            print("Reloading")
            continue
    ...
    if args.online:
        ...
        if not recv:
            print(
                "No marker stream found. Make sure there is a computer with a marker stream running"
            )
            sys.exit(0)
        clear()
